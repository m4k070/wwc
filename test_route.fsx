...
let pitchX = fsi.CommandLineArgs |> Array.tryItem 2 |> Option.map int |> Option.defaultValue 24
let pitchY = fsi.CommandLineArgs |> Array.tryItem 3 |> Option.map int |> Option.defaultValue 16
...
| Ok (grid, placed, pins) ->
    ...
    printfn "OK %s [%dx%d] %d cells (nand=%d dff=%d wire=%d cross=%d pin=%d) grid=%dx%d crossRatio=%.1f%% %dms"
        name pitchX pitchY total nc.Value dc.Value wc.Value cc.Value pc.Value gridW gridH
