...
1 は wgpu-runner `--memory` (2406b0a) で実現済み。2 は `--memory` の golden 照合 (B-3b、2026-09-16) で実現済み。
...
| sm83_subset の `INC r` / `DEC r` はレジスタ選択も誤り (`opcode[2:0]` を使う、2026-09-17 発見) | `INC A` (0x3C) が H を対象にする | 同上。直さない |
| sm83_full の RTL に不具合が多数 (2026-09-16〜17 修正) | 即値読出の off-by-one (09-16) に加え、仕様テストで 7 件 (前周期の operand で ALU/INC/DEC、H/C フラグの幅、POP/RET、CALL/RST の戻り番地、JP の pc 上書き、ALU (HL) の判定式) を発見・修正。さらに gbfs の CPU との全命令差分テスト (`src/DiffTestGbfs.fsx`) で 6 分類を修正し 494 命令一致。詳細 SM83.md | 手書き仕様テストと gbfs 差分テストを NetlistSim で回してから配線する |
