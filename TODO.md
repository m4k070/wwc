...
- [ ] sm83_full (10,767 gates) の配線完走 — 試行と結果:
      | 日付 | 条件 | 到達 | 結果 |
      |---|---|---|---|
      | 09-18 | 16x12 | 約 5,800 (32%) | 輻輳失敗 (72 分) |
      | 09-18 | 20x14 | 14,600 (81%) | 輻輳失敗 (90 分) |
      | 09-18 | 24x16 | 15,153 (84%) | 輻輳失敗 (93.6 分、NetId 165) |
      | 09-23 | 28x20 (案 A) | 16,000 (89%) | 輻輳失敗 (142.9 分、NetId 271) |
      | 09-23 | 28x20 + 撤去上限 80本/8回/5000 (案 B) | 16,016 (89%) | 輻輳失敗 (142.0 分、**同じ NetId 271**) |
      ...
      * → 混雑の原因は配置。案 C (配置改善) が本筋
- [ ] 案 C: 配置改善 — ネット構造の調査 (2026-09-23) と方式比較まで完了:
      * ネット構造: 最大ファンアウトは 181 (クロック) と 173 のみ、**93.8% がファンアウト 3 以下**。
        宣言順で前後 2 分割した cut は 1,031 (無作為分割 6,054 の 1/6) で、yosys の宣言順には強い局所性がある
      * 問題は並び順ではなく格子への敷き方。104x104 の行優先では、宣言順で半行ぶん離れただけで
        平面上は約 1,450 セル離れる。推定総配線長 1,390 万セル、1,000 セル超のネットが 32.3%
      * 方式比較 (28x20、推定総配線長 / 中央値 / >1000 の割合):
        行優先 1,390 万 / 140 / 32.3%、蛇行 1,325 万 / 132 / 30.4%、**ヒルベルト 1,297 万 / 68 / 25.1%**
      * ヒルベルト配置の実装は `placement-hilbert` ブランチに退避。**クロックスキュー均等化が効かなくなる**
        問題あり (reg8 で skew 36、本来 ≤1 → WL-SKEW テストが失敗)。DFF が平面に散り、クロック枝長を
        そろえにくくなるため。skew は CA の hold 違反に直結するので、この解決が前提
      * 次: (1) ヒルベルト配置でも skew を均等化できるようにする (DFF を曲線上でまとめる / クロック木の
        配線方式を見直す)、(2) その上で 28x20 で配線を試す
- [ ] CB 命令の動作検証

### 並行して進められる改善 (必須ではない)

- [ ] 配線時間の短縮 (ネット単位の並列化)。A* の同点処理で 24x16 は 93 分まで短縮済み。
      大規模回路から始めるピッチを 20x14 にして 16x12 の失敗分を省く案もある (`pitchFor`)
